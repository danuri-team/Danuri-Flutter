...
class ItemRentalViewModel {
  final ItemRentalDataSource _dataSource = ItemRentalDataSource();
  ...
  Future<void> itemRental(String usageId, String itemId, int quantity) async {
    try {
      _rentedItem = await _dataSource.itemRental(
        usageId,
  ...
  Future<void> getItemAvailableRent() async {
    try {
      _itemAvailableRental = await _dataSource.getItemAvailableRental();
      _error = false;
    } on DioException catch (_) {
      _error = true;
    }
